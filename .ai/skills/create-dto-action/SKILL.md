...
3. **Scaffolding**: Use `php artisan make:class` in the directory that matches the slice you are working in. Prefer `app/Modules/{Domain}/Actions/{Name}` for new modular work, but follow the existing location when extending a legacy slice that has not been migrated yet.
