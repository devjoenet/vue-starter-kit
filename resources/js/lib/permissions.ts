...
export function prefixPermissionWithGroup(group: string, actionSegment = ''): string {
...
export function extractPermissionActionSegment(permissionName: string, group: string): string {
  const normalizedGroup = toSnakeCase(group);
...
export function normalizePermissionName(permissionName: string, group: string): string {
