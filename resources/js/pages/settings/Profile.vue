      ...
      <div v-if="user" class="flex flex-col space-y-6">
        ...
        <Form v-bind="ProfileController.update.form()" class="space-y-6" v-slot="{ errors, processing, recentlySuccessful }">
          <div class="grid gap-2">
            ...
            <Input id="name" class="mt-1 block w-full" name="name" :default-value="user.name" required autocomplete="name" placeholder="Full name" :state="errors.name ? 'destructive' : 'default'" :message="errors.name" />
          </div>

          <div class="grid gap-2">
            <Label for="email">Email address</Label>
            <Input id="email" type="email" class="mt-1 block w-full" name="email" :default-value="user.email" required autocomplete="username" placeholder="Email address" :state="errors.email ? 'destructive' : 'default'" :message="errors.email" />
          </div>
