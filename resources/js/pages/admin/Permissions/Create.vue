...
import { extractPermissionActionSegment, inferPermissionLabel, normalizePermissionName, prefixPermissionWithGroup } from '@/lib/permissions';
...
setAdminBreadcrumbs({ title: 'Permissions', href: index.url() }, { title: 'Create', href: create.url() });
...
watch(
  ...
  (nextGroup, previousGroup) => {
    ...
    if (!form.name || !form.name.includes('.') || form.name.startsWith(`${previousGroupSlug}.`)) {
      const action = extractPermissionActionSegment(form.name || 'view', previousGroupSlug || normalizedGroup) || 'view';
      ...
      <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_18rem] xl:items-start">
        ...
        <AdminEditorAsideCard class="motion-step xl:self-start" style="--motion-order: 2">
          ...
          <p class="text-sm leading-6 text-muted-foreground">
            Treat the permission key like a durable API contract, then use the label and descriptions to make the catalog readable for the people assigning access. Group metadata should describe the broader business area, not a single one-off
            permission.
          </p>
