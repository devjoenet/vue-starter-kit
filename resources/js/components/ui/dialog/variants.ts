...
export const dialogCloseVariants = tv({
  base: "absolute top-4 right-4 rounded-xs opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  variants: {
    variant: {
...
export const dialogHeaderVariants = tv({ base: 'flex flex-col gap-2 text-center sm:text-left' });
export const dialogFooterVariants = tv({ base: 'flex flex-col-reverse gap-2 sm:flex-row sm:justify-end' });
export const dialogTitleVariants = tv({ base: 'text-lg leading-none font-semibold' });
export const dialogDescriptionVariants = tv({ base: 'text-sm text-muted-foreground' });
export const dialogScrollOverlayVariants = tv({
  base: 'fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[var(--overlay-backdrop)] data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0 motion-reduce:data-[state=closed]:animate-none motion-reduce:data-[state=open]:animate-none',
});
export const dialogScrollContentVariants = tv({
  base: 'relative z-50 my-8 grid w-full max-w-lg gap-4 border p-6 shadow-lg duration-200 motion-reduce:transition-none sm:rounded-lg md:w-full',
  variants: {
    variant: dialogFeedbackVariants,
  },
  defaultVariants: {
...
export const dialogScrollCloseVariants = tv({
  base: 'absolute top-4 right-4 rounded-md p-0.5 transition-colors',
  variants: {
    variant: {
