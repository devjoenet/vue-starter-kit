...
export const sidebarWrapperVariants = cva('group/sidebar-wrapper flex min-h-svh w-full has-data-[variant=inset]:bg-sidebar');
...
export const sidebarMenuSubVariants = cva('mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2.5 py-0.5');
...
export const sidebarContainerVariants = cva('flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground');
export const sidebarMobileContentVariants = cva('w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden');
export const sidebarDesktopGapVariants = cva('relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-linear motion-reduce:transition-none');
export const sidebarDesktopContainerVariants = cva('fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear motion-reduce:transition-none md:flex');
