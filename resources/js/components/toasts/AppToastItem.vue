...
const toastClasses = computed(() => {
  if (props.item.appearance === 'solid') {
    return cn('pointer-events-auto relative w-72 overflow-hidden rounded-[1.375rem] border pt-4 pr-4 pb-4 pl-4', appearanceClasses.solid, solidToneClasses[props.item.tone]);
  }
...
<template>
  <div :class="toastClasses" role="status" aria-live="polite">
    <button type="button" class="absolute top-2 right-2 flex size-10 items-center justify-center rounded-full opacity-70 transition hover:opacity-100" @click="$emit('dismiss', item.id)">
      <X class="size-4" />
    </button>

    <div class="flex items-start gap-3 pr-8">
      <span v-if="resolvedIcon" :class="cn('mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full', toneBadgeClasses[item.tone])">
        <component :is="resolvedIcon" class="size-4" />
      </span>

      <div class="min-w-0 pr-2">
        <p class="text-[0.68rem] font-semibold tracking-[0.16em] uppercase" :class="toneBadgeClasses[item.tone]">
          {{ toneLabelMap[item.tone] }}
        </p>
        <p v-if="item.title" class="mt-1 text-sm font-semibold">
          {{ item.title }}
        </p>
        <p class="mt-1 text-sm leading-6" :class="item.title ? 'opacity-95' : 'font-medium'">
          {{ item.message }}
        </p>
      </div>
    </div>
